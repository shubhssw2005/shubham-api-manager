...
echo "🚀 Building Ultra 10M RPS C++ System"
echo "===================================="
...
CFLAGS="-std=c++17 -O3 -march=native -mtune=native -flto -ffast-math"
CFLAGS="$CFLAGS -funroll-loops -finline-functions -fomit-frame-pointer"
CFLAGS="$CFLAGS -DNDEBUG -pthread -mavx2 -mfma"
...
if pkg-config --exists numa 2>/dev/null; then
    ...
    LIBS="$LIBS -lnuma"
    CFLAGS="$CFLAGS -DHAVE_NUMA"
else
    echo "⚠️  NUMA library not found, using fallback"
    CFLAGS="$CFLAGS -DNO_NUMA"
fi
...
if g++ $CFLAGS ultra_10m_rps_system.cpp -o ultra_10m_rps_system $LIBS 2>/dev/null; then
    echo "✅ Ultra-optimized version compiled successfully"
elif g++ -std=c++17 -O3 -pthread ultra_10m_rps_system.cpp -o ultra_10m_rps_system 2>/dev/null; then
    echo "✅ Standard optimized version compiled"
else
    echo "⚠️  Compilation failed, creating simplified version..."
    
    # Create simplified version without NUMA
    cat > ultra_10m_rps_simple.cpp << 'EOF'
#include <iostream>
#include <string>
#include <vector>
#include <thread>
...
EOF
    ...
    if g++ -std=c++17 -O3 -pthread ultra_10m_rps_simple.cpp -o ultra_10m_rps_system; then
...
echo "   ./ultra_10m_rps_system"
echo ""
echo "🚀 Run with:"
echo "   ./ultra_10m_rps_system"
echo ""
echo "💡 For best performance:"
echo "   sudo nice -n -20 ./ultra_10m_rps_system"
echo ""