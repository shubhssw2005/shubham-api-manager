...
resource "aws_eks_node_group" "memory_optimized" {
  ...
  tags = merge(var.tags, {
    Name = "${var.project_name}-${var.environment}-memory-nodes"
  })
}
...
# Update node groups with autoscaling tags
resource "aws_autoscaling_group_tag" "general_cluster_autoscaler_enabled" {
  for_each = toset([
    "k8s.io/cluster-autoscaler/enabled",
    "k8s.io/cluster-autoscaler/${aws_eks_cluster.main.name}"
  ])
...
resource "aws_autoscaling_group_tag" "memory_optimized_cluster_autoscaler_enabled" {
  for_each = toset([
    "k8s.io/cluster-autoscaler/enabled",
    "k8s.io/cluster-autoscaler/${aws_eks_cluster.main.name}"
  ])

  autoscaling_group_name = aws_eks_node_group.memory_optimized.resources[0].autoscaling_groups[0].name
